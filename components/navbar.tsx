...
export function Navbar() {
  ...
  return (
    <nav>
      <div className='flex flex-row w-full h-20 max-w-mdx items-center justify-between overflow-hidden'>
        ...
        <div className='z-40 hidden md:flex'>
          <ul className='flex flex-row items-center justify-center p-4'>
            {navLinks.map(link => (
              <motion.li
                className='p-4 transition duration-150 ease-in-out hover:text-foreground'
                ...
                <Link
                  ...
                  download={link.title === 'Resume' ? true : false}
                  aria-label={`Christian B Martinez | ${link.title}`}>
                  {link.title}
                </Link>
              </motion.li>
            ))}
          ...
          <div
            className={`z-40 mb-1 mr-4 flex h-6 w-10 cursor-pointer flex-row items-center md:mr-0 ${
              theme === 'light' && loaded
                ? 'justify-start'
                : 'justify-end'
            } px-0.5 py-1 border border-border border-solid rounded-md transition-all duration-150 bg-background text-foreground`}
        ...
        <motion.ul
          className={
            nav
              ...
              : 'fixed right-[-100%] top-0 z-40 flex h-screen w-full flex-col items-center justify-center  bg-slate-50 duration-1000 ease-in-out dark:bg-slate-950'
          }
          variants={ulVariant}
          ...
          {navLinks.map((link, i) => (
            <motion.li
              ...
              whileTap={{ scale: 0.95 }}>
              <Link
                className='text-2xl font-light text-slate-900 transition duration-150 ease-in-out hover:text-foreground dark:text-slate-200 dark:hover:text-foreground'
                href={link.href}
                role='link'
                target={link.title === 'Resume' ? '_blank' : '_self'}
